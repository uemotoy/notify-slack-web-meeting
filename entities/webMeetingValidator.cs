...
namespace dcinc.api.entities
{
  public class WebMeetingValidator : AbstractValidator<WebMeeting>
  {
    public WebMeetingValidator()
    {
      ...
      RuleFor(webMeeting => webMeeting.StartDateTime).NotNull().NotEmpty().GreaterThan(DateTime.Today).WithMessage("startDateTime is invalid. Please specify the date and time after today.");
