...
namespace dcinc.api.queries
{
  public class WebMeetingsQueryParameterValidator : AbstractValidator<WebMeetingsQueryParameter>
  {
    public WebMeetingsQueryParameterValidator()
    {
      ...
      RuleFor(webMeeting => webMeeting.FromDateUtcValue).LessThanOrEqualTo(webMeeting => webMeeting.ToDateUtcValue).When(webMeeting => webMeeting.FromDate != null).WithMessage("fromDate is invalid. Please specify a date before toDate.");
